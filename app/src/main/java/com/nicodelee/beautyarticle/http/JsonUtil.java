...
public class JsonUtil {
  ...
  public static <T> T jsonToMod(String jsonString, Class<T> clazz) {
    ...
    try {
      return JsonMapper.getInstance().getJsonMapper().readValue(jsonString, clazz);
    } catch (IOException e) {
      e.printStackTrace();
      return null;
    }
  ...
  public static <T> ArrayList<T> jsonToList(String jsonString, Class<T> clazz) {
    ...
    try {
      return JsonMapper.getInstance()
          .getJsonMapper()
          .readValue(jsonString, JsonMapper.getInstance()
              .getJsonMapper()
              .getTypeFactory()
   ...
   */
  @SuppressWarnings("unchecked") public static <T> T jsonToMod(String jsonString,
      JavaType javaType) {
    if (StringUtils.isEmpty(jsonString)) {
      return null;
    }
    try {
      return (T) JsonMapper.getInstance().getJsonMapper().readValue(jsonString, javaType);
    } catch (IOException e) {
      e.printStackTrace();
      return null;
    }
